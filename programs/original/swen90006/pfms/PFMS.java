...
public class PFMS {
    ...
    /**
     * The authentication status of an account: not authenticated,
    ...
    /**
     * Stores the current authentication status (AUTHENTICATED or NOT_AUTHENTICATED) for each
     * registered account, keyed by username.
     */
     ...
     * accounts and no issued tickets.
     */
    public PFMS() {
    ...
    /**
     * Authenticates a user using a username and password, then updates their authentication
     * status. The username may identify either a VEHICLE_OWNER account (a plate number) or an
     * ADMIN account.
     *
    ...
    public void login(String username, String password)
            throws NoSuchAccountException, IncorrectPasswordException {
    ...
    /**
     * Checks if the account with the given username is authenticated.
     * @param username The username to check.
     * @return {@code true} if the account is authenticated, false otherwise
     * @throws NoSuchAccountException if the username does not exist in the system
     */
    ...
    /**
     * Checks if at least one ADMIN account is currently authenticated, system-wide.
     * @return {@code true} if any registered ADMIN account is currently authenticated.
     */
    public boolean isAnyAdminAuthenticated() {
        for (Map.Entry<String, Role> entry : roles.entrySet()) {
            if (entry.getValue() == Role.ADMIN
                    && authenticationStatus.get(entry.getKey()) == AuthenticationStatus.AUTHENTICATED) {
                return true;
            }
        }
        return false;
